...
export class HttpClient {
  constructor(
    public base_url: string = process.env.NEXT_PUBLIC_BASE_URL ??
      "http://localhost:8000",
  ...
  Send<T>(
    ...
    const fetcherFn: Fetcher<Response<T>> =
      fetcher ??
      ((url: string) => fetch(url, options).then((res) => res.json()));
    const { data, error, isLoading, mutate } = useSWR<Response<T>>(
