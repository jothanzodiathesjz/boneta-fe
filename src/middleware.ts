...
export async function middleware(request: NextRequest) {
  ...
  const userData: DomainUserWithProfile | null = user
    ? new DomainUserWithProfile(JSON.parse(user))
    : null;
  ...
  if (
    pathname.startsWith("/processing/kurir") &&
    !userData.roles.includes("kurir")
  ) {
    return NextResponse.redirect(new URL("/processing/403", request.url));
  }
  if (
    pathname.startsWith("/processing/chasier") &&
    !userData.roles.includes("kasir")
  ) {
    return NextResponse.redirect(new URL("/processing/403", request.url));
  }
  if (
    pathname.startsWith("/processing/kitchen") &&
    !userData.roles.includes("kitchen")
  ) {
    return NextResponse.redirect(new URL("/processing/403", request.url));
  }
  ...
  if (
    pathname.startsWith("/processing/users") &&
    !userData.roles.includes("admin")
  ) {
    return NextResponse.redirect(new URL("/processing/403", request.url));
  }
  if (
    pathname.startsWith("/processing/menus") &&
    !userData.roles.includes("admin")
  ) {
    return NextResponse.redirect(new URL("/processing/403", request.url));
  }
  if (
    pathname.startsWith("/processing/others") &&
    !userData.roles.includes("admin")
  ) {
    return NextResponse.redirect(new URL("/processing/403", request.url));
  }
...
export const config = {
  matcher: ["/processing/:route*", "/kurir/:route*"],
};
