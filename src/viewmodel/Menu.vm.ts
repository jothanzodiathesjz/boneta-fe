...
const MenuViewModel = () => {
  ...
  const getData = () => {
    const { data, isError } = httpClient.Send<DomainMenu[]>(
      "/api/menu",
      undefined,
      undefined,
    );
  ...
  const { data, isError, isLoading, mutate } = httpClient.Send<DomainMenu[]>(
    "/api/menus",
    undefined,
    undefined,
    {
  ...
  const handleSendData = async () => {
    try {
      setOnLoading(true);
      const response = await httpClient.PostWithFile<DomainFile>(
        "/api/menu/file",
        file!,
      );
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (file) {
        const response1 = await httpClient.PostWithFile<DomainFile>(
          "/api/menu/file",
          file!,
        );
      ...
      const response = await httpClient.Put<DomainMenu>(
        `/api/menu/${menuForm.uuid}`,
        {
          body: JSON.stringify({
            ...menuForm,
            image: imgurl,
