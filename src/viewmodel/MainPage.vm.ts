...
export const MainPageViewModel = () => {
  ...
  const [selectedButton, setSelectedButton] = useState<DomainCategory | null>(
    null,
  );
  ...
  const { data, isError, isLoading } = http.Send<DomainMenu[]>(
  ...
  const handleSelectProduct = (item: DomainOrderItem) => {
    ...
    if (index !== -1) {
      ...
      if (orderToCart.length > 0) {
        setOrderToCart([
          ...orderToCart,
          { ...item, quantity: 1, total_price: item.price },
        ]);
      } else {
  ...
  const hanldeItemTocart = (item: DomainOrderItem) => {
    if (!orderToCart)
      setOrderToCart([new DomainOrderItem({ ...item, stage: 1 })]);
    else {
      const index = orderToCart?.findIndex((v) => v.uuid === item.uuid);
      if (index !== -1) {
        setOrderToCart((prev) => {
  ...
  const handleAdjustment = async () => {
    ...
    try {
      if (!oid) return;
      if (!order) return;
      const dataCart: DomainOrderItem[] =
        cartResult?.items.map((v) => {
          return {
            ...v,
